...
# 1. HTML: Add repuestos container and Aceptar button
...
content = re.sub(
    r'<div class="px-5 pb-5 flex gap-2">.*?<button id="btn-finalizar-cita" onclick="finalizarCitaActiva\(\)".*?Cerrar\s*</button>\s*</div>',
    new_buttons,
    content,
    flags=re.DOTALL
)

# 2. CITAS_DATA: Add repuestos
...
content = re.sub(
    r'estado:\s*"\{\{ cita\.estado\.nombre\|escapejs \}\}",\s*observaciones:"\{\{ cita\.observaciones\|default:\'\'\|escapejs \}\}"[ \t\n]*\}',
    new_cita_data,
    content,
    flags=re.DOTALL
)

# 3. JS Variables and logic
js_logic = '''
...
content = re.sub(
    r'const btnFinalizar = document\.getElementById\(\'btn-finalizar-cita\'\);.*?btnFinalizar\.className = \'flex-1 py-3 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-semibold text-sm transition shadow\';\s*\}?\s*\}',
    new_ver_detalle,
    content,
    flags=re.DOTALL
)

# 5. AceptarCita function
new_aceptar = '''
...
# 6. Finalizar cita body
content = re.sub(
    r'headers: \{\s*\'Content-Type\': \'application/json\',\s*\'X-CSRFToken\': csrfToken\s*\}',
...
print("Agenda patched flawlessly via regex!")
